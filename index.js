...
const mysql = require('mysql2'); // or 'mysql2' for the promise-based version
const app = express();
...
app.get('/api/products', (req, res) => {
  db.query('SELECT * FROM product;', (err, result) => {
    if (err) {
      console.log('error from products api')
...
app.get("/api/products/search", (req, res) => {
  ...
  if (!keywords) {
    return res.status(400).json({ error: "Keywords are required for the search from manish" });
  }
...
app.get("/api/products/sort", (req, res) => {
  ...
  // Check if choice parameter is provided
  if (!choice) {
    return res
      .status(400)
      .json({ error: "choice criteria (choice) is required" });
  }

  // Define a SQL query to choice products based on the specified criteria
  let query = "SELECT * FROM Product";

  // Add the ORDER BY clause based on the specified choice parameter
  switch (choice) {
    case "price":
...
app.get('/api/products/:product_id/details', (req, res) => {
  ...
  const query = 'SELECT pd.*, p.is_available FROM product_details pd JOIN product p ON pd.product_id = p.product_id WHERE pd.product_id = ?';
  db.query(query,[productId],(err, results) => {
      if (err) {
        console.error('Database query error:', err);
        res.status(500).json({ error: 'Internal server error' });
